...
def menambahkan_buku (no_isbn, judul, pengarang, isiHalaman, deskripsi, stok, booked):
    ...
    list_book.append(buku)
    print("Buku ditambahkan ke dalam daftar")

def daftar_buku():
    if not list_book:
        print("buku tidak tersedia")
    else:
        for buku in list_book:
...
def menambahkan_mahasiswa(nama, nim, kontak, alamat):
    ...
    list_student.append(mahasiswa)
    print("Mahasiswa ditambahkan ke dalam daftar")

def daftar_mahasiswa():
    if not list_student:
        print("mahasiswa tidak terdaftar")
    else:
        for mahasiswa in list_student:
...
def menambahkan_peminjam(nim, no_isbn, tanggal_pinjam, tanggal_kembali, status):
    ...
    print("Peminjaman ditambahkan ke dalam daftar")

def pinjam_buku(nim, no_isbn):
    mahasiswa = next((m for m in list_student if m['nim'] == nim), None)
    buku = next((b for b in list_book if b['no_isbn'] == no_isbn), None)
    ...
    if mahasiswa and buku:
        if buku['stok'] - buku['booked'] > 0:
            ...
            print('maaf buku tidak tersedia atau stok habis')
    else:
        print('maaf mahasiswa atau buku tidak ditemukan dalam daftar peminjaman')
